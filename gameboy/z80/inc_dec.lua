...
function apply(opcodes, opcode_cycles, z80, memory)
	...
	opcodes[0x04] = function()
		reg.b = band(reg.b + 1, 0xFF)
		set_inc_flags(reg.b)
	end
	opcodes[0x0C] = function()
		reg.c = band(reg.c + 1, 0xFF)
		set_inc_flags(reg.c)
	end
	opcodes[0x14] = function()
		reg.d = band(reg.d + 1, 0xFF)
		set_inc_flags(reg.d)
	end
	opcodes[0x1C] = function()
		reg.e = band(reg.e + 1, 0xFF)
		set_inc_flags(reg.e)
	end
	opcodes[0x24] = function()
		reg.h = band(reg.h + 1, 0xFF)
		set_inc_flags(reg.h)
	end
	opcodes[0x2C] = function()
		reg.l = band(reg.l + 1, 0xFF)
		set_inc_flags(reg.l)
	end
